...
class Py3status:
    ...
    def _color(self, input, config):
        if input <= self.med_threshold:
            return config['color_good']
        elif input <= self.high_threshold:
    ...
    def cpuInfo(self, i3s_output_list, i3s_config):
        ...
        response['color'] = self._color(100 * used_cpu_percent, i3s_config)
        #cpu temp
        CPUTEMP = False
        if CPUTEMP:
    ...
    def ramInfo(self, i3s_output_list, i3s_config):
        ...
        response['color'] = self._color(used_mem_percent, i3s_config)
        response['full_text'] = "RAM: %d%%" % (used_mem_percent)
        response['cached_until'] = time() + self.cache_timeout
